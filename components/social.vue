<template>
  <div id="social"
    class="flex flex-col min-h-screen justify-center items-center xl:items-stretch xl:justify-between gap-32 md:gap-16 py-10">
    ...
    <div class="hidden xl:block col-start-2 row-start-2 text-end">
      ...
      <Loader v-if="loading" />
      <a v-if="phoneNumber" :href="`tel:+39${phoneNumber}`" class="text-subtitle text-green underline">{{ phoneNumber
        }}</a>
    </div>
    <div class="xl:col-start-1 xl:row-start-3 col-span-2 max-w-sm">
      ...
      <h6 class="xl:hidden block text-center max-w-sm">But If you want to find out more, check out my social links to...
      </h6>
    </div>
      ...
      <ul class="list-none social-list">
        <li class="flex xl:items-baseline">
          <h3 class="hidden xl:flex xl:items-baseline">
            Read my
            <span class="hidden xl:inline px-6 text-[23px] tracking-[0.15px] font-medium">megalomaniacal</span>
          </h3>
          <h3 class="hidden xl:block pr-6">posts on</h3>
          <h3><a target="_blank" class="underline text-green hidden xl:block"
              href="https://www.linkedin.com/in/giovanni-rizzolo/">LinkedIn</a></h3>
          <div class="block xl:hidden text-center">Read my megalomaniacal posts on <a
              href="https://www.linkedin.com/in/giovanni-rizzolo/"
              class="inline-block xl:hidden text-green underline">LinkedIn</a></div>
        </li>
        <li class="flex justify-center xl:justify-start xl:items-center">
          <h3 class="hidden xl:block">Fix my broken code on <a target="_blank" class="underline text-green"
              href="https://github.com/giovannirizzolo">GitHub</a></h3>
          <div class="block xl:hidden text-center">Fix my broken code on <a href="https://github.com/giovannirizzolo"
              class="inline-block xl:hidden text-green underline">GitHub</a></div>
        </li>
        <li class="flex justify-center xl:justify-start xl:items-center">
          <h3 class="hidden xl:block">Like my pics on <a target="_blank" class="underline text-green"
              href="https://www.instagram.com/giovanni.rizzolo/">Instagram</a></h3>
          <div class="block xl:hidden text-center">Like my pics on <a href="https://www.instagram.com/giovanni.rizzolo/"
              class="underline text-green inline-block xl:hidden">Instagram</a></div>
        </li>
      </ul>
    </div>
    ...
    <Loader v-if="loading" />
    <a v-if="phoneNumber" :href="`tel:+39${phoneNumber}`" class="xl:hidden text-subtitle text-green underline">{{
      phoneNumber }}</a>
  </div>
</template>
